...
def run_vrp():
    ...
    route = openVRP_algorithm(locations, vehicles, depot_location)
...
def generate_solution(locations, solution, manager, routing, vehicles):
    ...
    for vehicle_id in range(vehicles):
        ...
        solution_str += 'Route for vehicle {}:<br>'.format(vehicle_id) + "&nbsp;"
        plan_output = ""
        route_distance = 0
...
def openVRP_algorithm(locations, vehicles, depot_location):
    ...
    distance_matrix = VRP_module.getDistanceMatrixOVRP(locations)
...
if __name__ == '__main__':
    run_simple('127.0.0.1', 41000, app, use_reloader=True)